...
class _Runtime:
	...
	lastWindowMoveTime: int = 0  # Track last window movement time for throttling
	...
	dirtyRects: list = None  # List of pygame.Rect for dirty rect updates
	_hudCacheKey: tuple = None  # Cache key for HUD to avoid unnecessary regeneration
	_headerNeedsRedraw = True  # Track if header needs redraw
	...
	def resetVars(self):
		...
		self.dirtyRects = []
		self._hudCacheKey = None
		self._headerNeedsRedraw = True
		self._staticMenuCache = None
		self.lastWindowMoveTime = 0
...
def moveWindow(mousePos):
	# Throttle window movement updates to reduce lag (update max 60 times per second)
	current_time = pygame.time.get_ticks()
	if current_time - Runtime.lastWindowMoveTime < 16:  # ~60 FPS max for window movement
		return
	Runtime.lastWindowMoveTime = current_time
	
	# Calculate new position
	new_x = Runtime.SDLwindow.position[0] - Runtime.windowGrabbedPos[0] + mousePos[0]
	new_y = Runtime.SDLwindow.position[1] - Runtime.windowGrabbedPos[1] + mousePos[1]
	Runtime.SDLwindow.position = (new_x, new_y)
...
def drawHeader():
	# Check if header hover states changed
	mouse_pos = pygame.mouse.get_pos()
	newMinimizeActive = Constants.HEADER_MINIMIZE_RECT.collidepoint(mouse_pos)
	newCloseActive = Constants.HEADER_CLOSE_RECT.collidepoint(mouse_pos)
	
	# Only redraw header if hover states changed or forced redraw
	if (Runtime._headerNeedsRedraw or 
	    Runtime.headerMinimizeActive != newMinimizeActive or 
	    Runtime.headerCloseActive != newCloseActive):
		header_rect = pygame.Rect(0, 0, Constants.SCREEN_WIDTH, Constants.HEADER_HEIGHT)
		...
		Runtime.headerMinimizeActive = newMinimizeActive
		...
		markDirty(header_rect)
		Runtime._headerNeedsRedraw = False
def drawHUD():
	assert IMG_HUD is not None
	Runtime.display.blit(IMG_HUD, Constants.HUD_RECT)
	markDirty(Constants.HUD_RECT)
...
def drawBackground(offset):
	background_rect = pygame.Rect(offset, Constants.HEADER_HEIGHT, Constants.SCREEN_WIDTH, Constants.SCREEN_HEIGHT - Constants.HEADER_HEIGHT)
	Runtime.display.blit(IMG_BACKGROUND, (offset, Constants.HEADER_HEIGHT))
	markDirty(background_rect)
...
def markDirty(rect):
	'''Marks a region as dirty for update'''
	if rect is not None:
		if isinstance(rect, tuple):
			rect = pygame.Rect(rect)
		Runtime.dirtyRects.append(rect)

def update():
	'''Updates the display using dirty rect optimization'''
	if Runtime.dirtyRects:
		# Merge overlapping rects for efficiency
		merged_rects = _mergeDirtyRects(Runtime.dirtyRects)
		pygame.display.update(merged_rects)
		Runtime.dirtyRects.clear()
	else:
		# Fallback to full update if no dirty rects tracked
		pygame.display.update()

def _mergeDirtyRects(rects):
	'''Merges overlapping dirty rects to reduce update calls'''
	if not rects:
		return []
	# Simple merging: if rects overlap significantly, merge them
	merged = []
	for rect in rects:
		merged_this_rect = False
		for i, existing in enumerate(merged):
			# If rects overlap or are close, merge them
			if existing.colliderect(rect) or existing.inflate(10, 10).colliderect(rect):
				merged[i] = existing.union(rect)
				merged_this_rect = True
				break
		if not merged_this_rect:
			merged.append(rect)
	return merged
def quit():
	pygame.quit()
...
def genHUD(options, shipSizes: dict[int, int], gameStage: STAGES, gameWon: bool, inTransition: bool):
	# Create cache key based on all parameters that affect HUD appearance
	cache_key = (
		gameStage,
		tuple(sorted(shipSizes.items())),  # Convert dict to sorted tuple for hashability
		options.opponentReady,
		options.myGridShown,
		gameWon,
		inTransition,
		tuple(sorted(Runtime.shipboxHovered)) if Runtime.shipboxHovered else (),  # Shipbox hover state
		options.submittedPlayerName() if hasattr(options, 'submittedPlayerName') else '',
		options.opponentName if hasattr(options, 'opponentName') else ''
	)
	
	# Only regenerate HUD if cache key changed
	if Runtime._hudCacheKey == cache_key:
		return  # HUD hasn't changed, skip regeneration
	
	Runtime._hudCacheKey = cache_key
	prepareImgHUD()
